...
{
    public partial class frmArqueoFinal : Form
    {
        ...
        float sistema, caja, arInicial;

        private void arqueo()
        {
            sistema = ClsArqueo.venta;
            lblSistema.Text = sistema.ToString();

            arInicial = ClsArqueo.arqueoInicial;
            lblFondo.Text = arInicial.ToString();

            caja = sistema + arInicial;
        ...
        private void btnSalir_Click(object sender, EventArgs e)
        {
            try
            {
                c.codigo = int.Parse(miclase.FolioSiguiente("codigo", "salirsistema").ToString());
                c.usuario = ClsMenu.rol;
                c.arqueo = float.Parse(lblCaja.Text);
                c.fecha = DateTime.Parse(DateTime.Now.ToString("dd/MM/yyyy"));

                if (c.salirsistema() == true)
                {
                    Application.Exit();
                }
                else
                {
                    MessageBox.Show("Error al ingresar el fondo");
        ...
        private void frmArqueoFinal_Load(object sender, EventArgs e)
        {
            arqueo();
        }
        ...
        private void frmArqueoFinal_Resize(object sender, EventArgs e)
        {
            AdjustForm();
        }
