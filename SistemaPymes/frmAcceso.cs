...
namespace SistemaPymes
{
    public partial class frmAcceso : Form
    {
        ...
        private void btnaccesar_Click(object sender, EventArgs e)
        {
            ...
            {
                rol = c.ObtenerRol(username);
                if (rol != null)
                {
                    MessageBox.Show("¡Bienvenido " + rol + "!");

                    frmArqueoInicial ini = new frmArqueoInicial();
                    ini.Show();
                    Hide();
        ...
        {
            if (e.KeyChar == 13) btnAccesar.Focus();
            if (e.KeyChar == 27)
            {
                //txtPassword.Clear();
                txtUsuario.Focus();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                // Obtener y mandar los datos guardados en un archivo (Se manda a frmImpTicket)
                using (System.IO.StreamReader file = new System.IO.StreamReader(@"impticket.rar"))
                {
                    string line;
                    while ((line = file.ReadLine()) != null)
                    {
                        ...
                        if (parts.Length >= 2)
                        {
                            ...
                            {
                                case "impTickets":
                                    frmEmpresa.impTickets = parts[1].Trim();
                                    break;
                                    // Agrega más casos si hay más propiedades a asignar
                            }
                        }
                    }
                    file.Close();
        ...
        private void frmAcceso_Resize(object sender, EventArgs e)
        {
            AdjustForm();
        }
    }
