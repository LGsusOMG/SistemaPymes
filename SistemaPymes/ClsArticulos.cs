...
namespace SistemaPymes
{
    class ClsArticulos
    {
        ...
        public bool Guardar()
        {
            ...
            MySqlCommand cmd = new MySqlCommand("Insert into articulos values (@co,@ds,@pc,@pv,@ex,@sm,@sma)", cn);
            cn.Open();
        ...
        {
            MySqlConnection cn = new MySqlConnection(miclase.conexion);
            MySqlCommand cmd = new MySqlCommand("Update articulos set descripcion=@ds,pcompra=@pc,pventa=@pv,existencia=@ex,sminimo=@sm,smaximo=@sma where codigo=@co", cn);
            cn.Open();
            cmd.Parameters.AddWithValue("@co", this.co);
            cmd.Parameters.AddWithValue("@ds", this.ds);
            cmd.Parameters.AddWithValue("@pc", this.pc);
