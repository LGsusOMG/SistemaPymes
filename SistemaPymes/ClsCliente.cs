...
namespace SistemaPymes
{
    ...
    class ClsCliente : ClsPersona
    {
        ...
        {            
            ...
            MySqlCommand cmd = new MySqlCommand("Insert into clientes values(@co,@rf,@no,@di,@te,@em)",cn);
            cn.Open();
        ...
        {
            MySqlConnection cn = new MySqlConnection(miclase.conexion);
            MySqlCommand cmd = new MySqlCommand("Update clientes set rfc=@rf,nombre=@no,direccion=@di,telefono=@te,email=@em where codigo=@co", cn);
            cn.Open();
            cmd.Parameters.AddWithValue("@co", this.co);
            cmd.Parameters.AddWithValue("@rf", this.rf);
            cmd.Parameters.AddWithValue("@no", this.no);
